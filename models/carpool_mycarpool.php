<?php
    ...
    class carpool_mycarpool
    {
        ...
        {
            return $this->connect->sql_query("select  Account from User_AC_PW
        ...
        {
            $this->connect->sql_query("update User_AC_PW set 
            $ID=0 where Account='{$_POST['Account']}'");
            if($_POST['type']=='passenger')
                $this->connect->sql_query("update Carpool_data set ps_boolean=0 where ID='{$_POST['ID']}'");
            else
        ...
        function master_quit()    //如果該使用者是主揪，則刪除所有此次共乘資訊
        {
            ...
            $this->connect->sql_query("DELETE FROM `Carpool_data_plus` where ID='{$_POST['ID']}' ");
            $this->connect->sql_query("update User_AC_PW set ID1=0 where ID1='{$_POST['ID']}'");
            $this->connect->sql_query("update User_AC_PW set ID2=0 where ID2='{$_POST['ID']}'");
            $this->connect->sql_query("update User_AC_PW set ID3=0 where ID3='{$_POST['ID']}'");
        }
        function lack_reduce($ID)   //更新該使用者的ID，並依照TYPE去更新該次共乘資訊
        {
            $this->connect->sql_query("update User_AC_PW set $ID='{$_POST['ID']}' where Account='{$_POST['Account']}'");
		    ($_POST['type']=='driver')?
    			$this->connect->sql_query("update Carpool_data SET lack=lack-1 
    			where ID='{$_POST['ID']}'"):
    			$this->connect->sql_query("update Carpool_data SET ps_boolean=1 
    			where ID='{$_POST['ID']}'");
        }
